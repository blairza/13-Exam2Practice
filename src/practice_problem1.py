...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        # ---------------------------------------------------------------------
        place = self.contents + additional_contents
        if(len(self.contents + additional_contents) > self.volume):
            val = len(self.contents + additional_contents) - len(self.contents)
            print(val)
            if(val+1 < self.volume):
                for k in range(len(self.contents), self.volume):
                    self.contents = self.contents + place[k]
                for k in range(self.volume, len(place)):
                    self.s = self.s + place[k]
            else:
                for k in range(val + 1, len(self.contents + additional_contents)):
                    self.s = self.s + place[k]
                for k in range(len(self.contents), val + 1):
                    self.contents = self.contents + place[k]
        else:
    ...
    def shrink(self, new_volume):
        ...
        if(self.volume < len(self.contents)):
            for k in range(0, self.volume):
                place1 = place1 + place[k]
        else:
            for k in range(len(self.contents)):
                place1 = place1 +place[k]
        self.contents = place1
        return self.s
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # ---------------------------------------------------------------------
        self.hist.append(self.contents)
